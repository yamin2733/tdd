...
class ItemValidationTest(FunctionalTest):
	...
	def test_error_messages_are_cleared_on_input(self):
    	 ...
    	 self.get_item_input_box().send_keys('Banter too thick')
    	 self.get_item_input_box().send_keys(Keys.ENTER)
    	 self.wait_for(lambda: self.assertTrue(self.browser.find_element(By.CSS_SELECTOR,'.has-error').is_displayed()  ))

    	 self.get_item_input_box().send_keys('a')
    	 self.wait_for(lambda: self.assertFalse(self.browser.find_element(By.CSS_SELECTOR,'.has-error').is_displayed()  ))